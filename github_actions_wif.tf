...
resource "google_service_account_iam_member" "wif_impersonate" {
  ...
  member         = "principalSet://iam.googleapis.com/locations/global/workloadIdentityPools/github-pool/attribute.repository/${var.github_repo}"
}
...
resource "google_service_account_iam_member" "wif_token_creator_pool" {
  service_account_id = google_service_account.github_actions_deploy.name
  role               = "roles/iam.serviceAccountTokenCreator"
  member             = "principalSet://iam.googleapis.com/locations/global/workloadIdentityPools/github-pool"
}
